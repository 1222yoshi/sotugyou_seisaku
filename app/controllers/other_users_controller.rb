  ...
  def index
    ...
    if current_user && current_user.user_albums.present?
      ...
      if recent_users.any?
        ...
        content += "私の好きな音楽は#{current_user_likes}です。最大9つあったと思いますが最後まで使うので完全に記憶してください。\n"
        content += "これから他のユーザー#{user_count}人分の好きな音楽を1人につき最大9つ、「ユーザーID: user_id, 音楽: 'アーティスト名'の'アルバム名'（ID: album_id）」という形で送ります。\n"
        ...
        content += "まずお互いのアルバムを9枚ずつ最後まで見て、私と他のユーザーで「音楽: 'アーティスト名'の'アルバム名'」まで一致したら一つにつき80点与えてください。私もしくは他のユーザーのどちらかがその'アーティスト名'を他にも加えている、もしくは他の「音楽: 'アーティスト名'の'アルバム名'」が一致していたら100を与え、match_scoreの計算を終了してください。\n"
        content += "その後、私と他のユーザーで一つ「音楽: 'アーティスト名'」が一致したら60加点、私もしくは他のユーザーどちらかがその'アーティスト名'を二つ以上、もしくはその'アーティスト名'を除いてもほかの'アーティスト名'が一致したらで無条件で100をあたえ、match_scoreの計算を終了してください。\m"
        content += "それ以外の場合でもビートルズとオアシスのように違うアーティストでも音楽性や界隈、ルーツが近ければそれに準じた点数をつけてください、EDMとクラシックのように全く関係のない1点は妥当ですが、EDMとロックなら5点、EDMとソウルなら10点、同じジャンル同士なら20点、同じジャンルで年代や国が同じなら40点とある程度幅のある点数を与えてください。\n"
        content += "9枚の平均基準同士のマッチというより、一枚一枚総当たりの加点方式のような基準で点数をつけてください。（つまり音楽が1種類しかないユーザーより、音楽が9種類あるユーザーを優遇してください）同じアーティストの組み合わせでユーザーごとに点数のばらつきが出ないように採点基準の一貫性を強く持ってください。\n"
  ...
  def show
    ...
    if current_user && current_user.user_albums.present? && current_user != @user
      ...
      if last_updated_time.nil? || (other_user_updated_times.present? && other_user_updated_times > last_updated_time) || current_user_updated_time > last_updated_time
        ...
        content = "この文章を読んだ時、あなたは過去全ての私との会話の記憶を忘れて、以下の指示通りに一から作業してください。\m"
        content += "私の好きな音楽は#{current_user_likes}です。最大9つあったと思いますが最後まで使うので完全に記憶してください。\n"
        content += "これから他のユーザーの好きな音楽を最大9つ、「ユーザーID: user_id, 音楽: 'アーティスト名'の'アルバム名'（ID: album_id）」という形で送ります。\n"
        content += "その情報から私と他のユーザーとのマッチ度とその相手のユーザーの最大9枚のアルバムの中で一番私の音楽性に近いアルバムのidを一つ、以下の条件をよく読んで、【出力形式:】より後に書いてあるの配列の形で返してください。それ以外の発言は絶対にしないでください。\n"
        content += "マッチ度の条件:\n"
        content += "マッチ度はmatch_scoreとして後述する条件を除いて絶対に1から100の範囲で返してください。\n"
        content += "まずお互いのアルバムを9枚ずつ最後まで見て、私と他のユーザーで「音楽: 'アーティスト名'の'アルバム名'」まで一致したら一つにつき80点与えてください。私もしくは他のユーザーのどちらかがその'アーティスト名'を他にも加えている、もしくは他の「音楽: 'アーティスト名'の'アルバム名'」が一致していたら100を与え、match_scoreの計算を終了してください。\n"
