class ChatroomsController < ApplicationController
  ...
  def create
    ...
    @chatroom = Chatroom.where('user_1_id = ? AND user_2_id = ? OR user_1_id = ? AND user_2_id = ?', current_user.id,
                               @other_user.id, @other_user.id, current_user.id).first
  ...
  def show
    ...
    redirect_to root_path unless @chatroom.user_1_id == current_user.id || @chatroom.user_2_id == current_user.id
    @match = Match.find_by(user_id: current_user.id, other_user_id: @chatroom.other_user(current_user).id)
    Notification.where(user_id: current_user.id, source_user_id: @chatroom.other_user(current_user).id,
                       notification_type: 'message').update_all(is_read: true)
  ...
  def post_message
    ...
    img = if current_user.profile_image_url.nil?
            'me-4414548e6f4cf95a9be7fb8c2b780005f376419ca47bf32f6d6a185a7d472db6.png'
          else
    ...
    if @message.save
      ...
      ActionCable.server.broadcast "message_#{@chatroom.other_user(current_user).id}",
                                   { message: @message, other_user_img: img, other_user_id: current_user.id }
      Notification.create(user_id: @chatroom.other_user(current_user).id, source_user_id: current_user.id,
